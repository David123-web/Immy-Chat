import { CheckOutlined, CloseCircleOutlined, CloseOutlined, ReloadOutlined } from "@ant-design/icons";
import interact from "interactjs";
import React, { useEffect, useRef, useState } from "react";
import styles from "./DragAndDrop.module.css";

const DragAndDropDemo = ({ words, images, demoRef, resetDrillProgress }) => {
  try {
            ...
            interact.modifiers.snap({
              targets: [function (x, y, interaction) {
                let foundIndex = wordArrayDropCoordinates.findIndex(
                  (ele) => {
                    return Math.abs(x - ele.x) <= 50 && Math.abs(y - ele.y) <= 50 &&
    ...
    return (
      <>
        <div className={styles.parentBox}>
          ...
          <br></br>
          <h2 className={[clicked ? undefined : styles.hide, styles.correctCount].join(' ')}>{numOfCorrect}</h2>
          <ReloadOutlined

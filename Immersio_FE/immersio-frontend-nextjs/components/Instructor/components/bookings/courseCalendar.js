...
const CourseCalendar = ({ items }) => {
	...
	let arrDistinct = Array.from(new Set(items.map(x => x?.user_detail[0]?.id))).map(id => { return { id: id, arrStaff: items.find(y => y?.user_detail[0]?.id == id)?.user_detail } });
	...
	return(
		<div>
			<div className='calendar-container'>
				...
				<div>
				<Select className='select-calendar' placeholder="Choose a student" tokenSeparators={[,]} size='large'>
	           		{/* <Option key={1} value='Abraham Josiah Chappell'>Abraham Josiah Chappell</Option> */}
					{ arrDistinct.map((item, i) => (
	            		<Option key={i} value={item?.id}>{item?.arrStaff[0]?.lastName + ' ' + item?.arrStaff[0]?.firstName}</Option>
	            	))}
				</Select>

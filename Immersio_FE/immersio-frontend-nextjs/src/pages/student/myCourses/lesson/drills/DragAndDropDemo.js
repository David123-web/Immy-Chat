...
const DragAndDropDemo = ({ words, images, demoRef, resetDrillProgress, drill_id, index: propsIndex }) => {
	try {
		...
		const checkAnswer = async () => {
			...
			shuffledRef.current.forEach((ele, index) => {
				let leftCoorWord = document?.querySelector('#wordDiv' + index)?.getBoundingClientRect().left + window.scrollX;
				let topCoorWord = document?.querySelector('#wordDiv' + index)?.getBoundingClientRect().top + window.scrollY;
				let leftCoorDrop = document?.querySelector('#imageDiv' + index)?.getBoundingClientRect().left + window.scrollX;
				let topCoorDrop =
					document?.querySelector('#imageDiv' + index)?.getBoundingClientRect().top +
					window.scrollY +
					document?.querySelector('#imageDiv' + index)?.clientHeight -
					document?.querySelector('#wordDiv' + index)?.clientHeight +
					10;
		...
		const setSnapCoordinates = () => {
			...
			wordArray.forEach((ele, index) => {
				let leftCoordinate =
					document?.querySelector('#imageDiv' + index)?.getBoundingClientRect().left + window.scrollX;
				let topCoordinate =
					document?.querySelector('#imageDiv' + index)?.getBoundingClientRect().top +
					window.scrollY +
					document?.querySelector('#imageDiv' + index)?.clientHeight -
			...
			shuffledRef.current.forEach((e, index) => {
				...
				let selfLeftCoor = document?.querySelector('#wordDiv' + index)?.getBoundingClientRect().left + window.scrollX;
				let selfTopCoor = document?.querySelector('#wordDiv' + index)?.getBoundingClientRect().top + window.scrollY;
						...
						interact.modifiers.snap({
							targets: [
								function (x, y, interaction) {
									...
									if (foundIndex !== -1) {
										return {
											x: wordArrayDropCoordinates[foundIndex].x,
											y: wordArrayDropCoordinates[foundIndex].y,
											range: Infinity,
										};
									} else {
										return startingPos;
			...
			<>
				<div className={styles.parentBox}>
					{shuffledImages.map((ele) => {
						return (
							<div order={ele.index} className={[styles.imageBox, 'imageCards'].join(' ')} id={'imageDiv' + ele.index}>
						...
						return (
							<div
								...
								className={[
									...
									'wordCards',
									'draggable',
									correctArray[i] === true ? styles.correct : correctArray[i] === false ? styles.wrong : styles.no,
								].join(' ')}
								key={ele.index}
...
export default observer(DragAndDropDemo);

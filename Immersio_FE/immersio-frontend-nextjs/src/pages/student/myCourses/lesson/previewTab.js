...
import { LessonContext } from '../../../../../components/PreviewLesson';
import AudioPlay from '../../../../../components/PreviewLesson/components/audioPlay';
import DialogPreviewType from '../../../../../components/PreviewLesson/components/DialogPreviewType';
import Rewarding from '../../../../../components/PreviewLesson/components/Rewarding';
import IMAGE_LESSON_FLOW from '../../../../../constants/lessons-icon';
import { getTrackingCourseAndLessonID, updateTrackingCourseByID } from '../../../../services/courses/apiCourses';
import { formatSumDrills } from '../../../../utilities/helper';
import DrillPreview from './drills/index';
import styles from './style.module.css';
import ReactPlayer from 'react-player';
import PageSlideIntroduction from '@/components/Courses/SlideIntroduction/PageSlideIntroduction';
import { globalStore } from '../../../../stores/global/global.store';
import { observer } from 'mobx-react-lite';
import { resetLessonsProgress } from '../../../../services/lessons/apiLessons';

const LessonPreviewIndex = ({
	contextAudio,
	dataCourseSlide,
  ...
  console.log('====LessonPreviewIndex=')
		...
		if (lesson_id && course_id) {
			...
			if (response?.data?.length) {
				...
				if (newFormatIndexStep >= sumTotal - dialogs?.length) {
					...
					const formatGrammarSum =
						vocabulary?.length +
						formatSumDrills(vocabularyDrills) +
						phrases?.length +
						formatSumDrills(phrasesDrills) +
						1;
					const formatGrammarDrillSum =
						vocabulary?.length +
						formatSumDrills(vocabularyDrills) +
						phrases?.length +
						formatSumDrills(phrasesDrills) +
					...
					if (Array.from({ length: vocabulary?.length }, (_, i) => i + 1).includes(newFormatIndexStep)) {
						...
						Array.from({ length: formatSumDrills(vocabularyDrills) }, (_, i) => i + formatVocalDrillSum).includes(
							newFormatIndexStep
						)
					) {
						...
						Array.from({ length: phrases?.length }, (_, i) => i + formatPhraseSum).includes(newFormatIndexStep)
					) {
						setType('phrases');
						setProgress({
						...
						Array.from({ length: formatSumDrills(phrasesDrills) }, (_, i) => i + formatPhraseDrillSum).includes(
							newFormatIndexStep
						)
					) {
						...
						Array.from({ length: grammar?.length }, (_, i) => i + formatGrammarSum).includes(newFormatIndexStep)
					) {
						setType('grammar');
						setProgress({
						...
						Array.from({ length: formatSumDrills(grammarDrills) }, (_, i) => i + formatGrammarDrillSum).includes(
							newFormatIndexStep
						)
					) {
	...
	const processDragAndDrop = (elementParent, vocabs) => {
		...
		if (length > groupSize) {
			...
			if (remainder === 1) {
				...
				for (let j = (quotient - 1) * groupSize; j < quotient * groupSize - 1; j++) {
	...
	const handleNextPrevProgress = (changeType) => {
		...
		if (changeType === 'next') {
			...
			if (
				(cardTypes.indexOf(type) !== -1 && progress.id === 0) ||
				(cardTypes.indexOf(type) === -1 && drillSectionIndex === 0 && drillProgress.id === 0)
			) {
				switch (type) {
					case 'vocabularyDrills':
						setType('vocabulary');
		...
		switch (type) {
			...
			case 'vocabularyDrills':
				if (vocabularyDrills?.length) {
					setValue(
						vocabularyDrills.map((element, index) => {
							return { ...element, id: index };
						})
					);
				}
			...
			case 'phrasesDrills':
				if (phrasesDrills?.length) {
					setValue(
						phrasesDrills.map((element, index) => {
							return { ...element, id: index };
						})
					);
				}
			...
			case 'grammarDrills':
				if (grammarDrills?.length) {
					setValue(
						grammarDrills.map((element, index) => {
							return { ...element, id: index };
						})
					);
				}
			...
			<div className="animated fadeIn">
				{isDialogType ? (
					...
					<>
						{type === 'introduction' ? (
							<>
								<div className={`step-custom ${styles.step} ${styles.step02}`}>
									<div className={`step02 ${styles.stepVideoWrapper}`} style={{ width: 725, margin: 'auto' }}>
										{values.video && values.lessonObj.showIntroductionVideo ? (
											<>
										...
										) : dataCourseSlide.length > 0 && values.lessonObj.showDialogSlide ? (
											<PageSlideIntroduction
								...
								<div className={styles.actions}>
									<Tooltip title={vocabulary?.length === 0 ? 'Loading ...' : ''}>
										<div
							...
							<div className={`step-custom ${styles.step}`}>
								{value.length && cardTypes.indexOf(type) !== -1 ? (
									<>
										<div className={styles.stepTabs}>
											{value.map((session) => (
												<div
													key={`list-tab-${session.id}`}
													className={[styles.stepTabItem, session.id === progress.id ? styles.active : null].join(' ')}
												/>
											))}
										</div>
											...
											{value.map((session, index) => {
												...
												return (
													<div key={index}>
																...
																{progress.step === 2 ? (
																	<>
																		{session.video ? (
																			...
																			<img className={styles.image} src={session.image}></img>
																		) : null}
																	</>
																) : null}
															</div>
															<div className="tw-flex tw-items-center tw-mt-2 tw-w-[600px] tw-relative">
																<div
																	className={`tw-flex-1 tw-h-[60px] tw-px-[60px] tw-justify-center tw-items-center tw-flex ${[
																		styles.title,
																		progress.step >= 1 ? styles.custom : null,
																	].join(' ')}`}
																>
																	{session.input}
																</div>
														...
														{progress.step === 2 ? (
															<div
																className={styles.stepCardExplain}
									...
									<>
												...
												return (
													<div key={`list-drill-introduction-${element.id}`} className={styles.stepIntroduction}>
														{element.drillType === 'sort' && element.questions?.length ? (
															...
															<>
																{element.drillType === 'flashcards' && element.questions?.length
																	? element.questions[drillSectionIndex]
																	: element.instruction}
															</>
														)}
										...
										<div className={styles.stepTabs}>
											{value.map((session) => (
												<div
													key={`list-drill-tab-${session.id}`}
													className={[styles.stepTabItem, session.id === drillProgress.id ? styles.active : null].join(
														' '
													)}
												/>
											))}
								...
								<div className={styles.actions}>
									<div
										className={[styles.arrowLeft, isDisabledBtnPrev ? styles.disabled : undefined].join(' ')}
										onClick={() => {
											handleNextPrevProgress('prev');
									...
									>
										...
										(cardTypes.indexOf(type) === -1 && drillProgress.step === 1) ? (
											<Image src={IMAGE_LESSON_FLOW.eyelash} width={40} height={40} />
										) : null}
										{(cardTypes.indexOf(type) !== -1 && progress.step === 2) ||
										(cardTypes.indexOf(type) === -1 && drillProgress.step === 2) ? (
											<Image src={IMAGE_LESSON_FLOW.eyeopen} width={40} height={40} />
										) : null}
									</div>
									<div
										className={[
											styles.arrowRight,
											isDisabledBtnNext || globalStore.disableNextDrills ? styles.disabled : undefined,
										].join(' ')}
										onClick={() => {
											handleNextPrevProgress('next');
											globalStore.setDisableCheckAnswerAgain(false);
										}}
									>
										<RightOutlined />
...
export default observer(LessonPreviewIndex);

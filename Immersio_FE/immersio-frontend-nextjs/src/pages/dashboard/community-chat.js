...
import { connectWithSocketServer, getSocketConnection } from '../../../realTimeCommunication/socketConnection'
...
const CommunityChat = () => {
	...
	useEffect(() => {
		if (!getSocketConnection()) {
			console.log('Socket is disconnected. Contact support.')
			if (userStore.currentUser) {
				const user = JSON.parse(JSON.stringify(userStore.currentUser.id))
	...
	const sendFriendInvitation = async() => {
		try {
			let { data } = await http.post(`${process.env.NEXT_PUBLIC_EXTERNAL_API_URL}/service/community-chat/invite-friend`, { friendMail })
			console.log(data)
		} catch (err) {
			console.log(err)
	...
	const acceptFriendInvitation = async(invitation) => {
		try {
			let { data } = await http.post(`${process.env.NEXT_PUBLIC_EXTERNAL_API_URL}/service/community-chat/accept-invitation`, { invitation })
			console.log(data)
		} catch(err) {
			console.log(err?.response?.data)
			toast.error(err?.response?.data)
	...
	const rejectFriendInvitation = async(invitation) => {
		try {
			...
			let { data } = await http.post(`${process.env.NEXT_PUBLIC_EXTERNAL_API_URL}/service/community-chat/reject-invitation`, { invitation })
